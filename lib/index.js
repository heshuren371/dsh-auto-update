...
import { capture, killChild, readJson, resolveGitBin, runBash, sh, sleep, writeJsonAtomic } from './util.js'
import {
...
function bestDetail(lines) {
  ...
  const text = cleaned.find((line) => /error|fatal|not agreed|license/i.test(line))
...
function toolchainProblem() {
  ...
  try {
    ...
    if (result.status !== 0 && /Xcode license|not agreed to the Xcode license/i.test(output)) {
      return 'Xcode 许可尚未接受，原生模块无法编译。请先在终端执行：sudo xcodebuild -license accept'
    }
  } catch {
...
async function check() {
  ...
  if (code !== 0) {
    const detail = bestDetail(fetchErr)
 ...
 * @param options.stderr - 传入数组时，把 stderr 行同时收集进去（用于把真实失败原因写进错误信息）。
 */
async function stepBash(script, { cwd, timeoutMs = PIPELINE_TIMEOUT_MS, stderr = null } = {}) {
  ...
  const code = await runBash(script, {
    ...
    onErrLine: (line) => {
      appendLog(line)
      if (Array.isArray(stderr)) stderr.push(line)
    },
    onSpawn: (child) => {
...
async function runUpdate(runId) {
  ...
  try {
    ...
    if (installCode !== 0) {
      const detail = bestDetail(installErr)
    ...
    if (buildCode !== 0) {
      const detail = bestDetail(buildErr)
