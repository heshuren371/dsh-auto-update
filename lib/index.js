...
function scheduleRestart() {
  ...
  const script = [
    ...
    'for i in $(seq 1 100); do kill -0 ' + String(pid) + ' 2>/dev/null || break; sleep 0.1; done',
    `kill -9 ${pid} 2>/dev/null || true`,
    `cd ${sh(cwd)}`,
    `nohup ${sh(process.execPath)} ${sh(entry)} web --no-open >> ${sh(logPath)} 2>&1 &`,
  ].join('; ')
  try {
  ...
  return { ok: true, logPath, command: `${process.execPath} ${entry} web` }
}
