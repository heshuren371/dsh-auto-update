...
/**
 * 单次命令 stderr 的收集上限（保留最后约 200 行）。
 * pnpm install / build 能刷出几万行 stderr，这些行只用于 bestDetail 挑错，
 * 全留在数组里既吃内存又拖慢错误路径；关键行由 createLineBuffer 优先保留。
 */
...
/**
 * 更新副本策略：**每次更新都新建一个全新 worktree**，绝不复用旧槽位。
 *
 * 复用会带来两类真实事故（都实际发生过）：
 *   1) 旧版本的编译产物残留 —— 0.1.7-alpha.1 构建报
 *      `MISSING_EXPORT "SettingsProvider"`（settings-file 的旧 lib/types 还在）；
 *   2) 旧 node_modules 工作区状态污染 tsdown 的包解析 —— 报
 *      `[@deepseek-ai/dsh-root] Cannot find entry: lib/types/{index,invariant,startup}.js`。
 * 全新 worktree + 全新 pnpm install（有 pnpm store，通常几秒）从构造上消除这两类问题。
 * 保留数量：active（正在跑）+ candidate（新副本）+ 最近一次失败的副本（留给 agent 排查）。
 */
const MAX_KEPT_SLOTS = 3
/** 一次更新里最多自动禁用多少个不兼容插件。 */
const MAX_QUARANTINE = 8
/** 试运行（含隔离重试）的最大次数：每轮至少隔离一个新条目，这里再兜一层死循环。 */
const MAX_CANARY_ATTEMPTS = MAX_QUARANTINE + 1
/**
 * 试运行「快速失败」的重试次数。
...
/** 失败报告：结构化 JSON + 日志尾部，供界面/CLI/agent 接手。 */
const FAILURES_DIR = path.join(STATE_DIR, 'failures')
const FAILURE_FILE = path.join(FAILURES_DIR, 'latest.json')
const FAILURE_LOG = path.join(FAILURES_DIR, 'latest.log')
/** 整条流水线的完整日志（有上限），失败时截取尾部写进失败报告。 */
const PIPELINE_LOG = path.join(STATE_DIR, 'pipeline.log')
const MAX_PIPELINE_LOG_BYTES = 8 * 1024 * 1024
/** 失败报告里保留的日志行数。 */
const MAX_FAILURE_TAIL_LINES = 160
/** 「让 dsh 帮忙修复」用的 profile 与状态文件。 */
const ASSIST_FILE = path.join(STATE_DIR, 'assist.json')
const ASSIST_PROFILE = process.env.DSH_UPDATE_ASSIST_PROFILE ?? 'headless'
...
const state = {
  ...
  canary: null,
  /** 最近一次失败的摘要（完整报告在 failures/latest.json）。 */
  failure: null,
  /** 「让 dsh 帮忙修复」的会话状态。 */
  assist: null,
 ...
 * 整条流水线的完整日志落盘。
 *
 * 内存环形缓冲只有 400 行 / 256KB，只够界面看；失败时要把足够的上下文交给
 * 模型/agent 排查，所以另开一个 8MB 上限的顺序文件，失败时截尾部写进失败报告。
 */
let pipelineLogStream = null
let pipelineLogBytes = 0

function openPipelineLog() {
  closePipelineLog()
  pipelineLogBytes = 0
  try {
    mkdirSync(STATE_DIR, { recursive: true })
    pipelineLogStream = createWriteStream(PIPELINE_LOG, { flags: 'w' })
    pipelineLogStream.on('error', () => { pipelineLogStream = null })
  } catch {
    pipelineLogStream = null
  }
}

function recordPipelineLine(text) {
  if (pipelineLogStream === null) return
  if (pipelineLogBytes <= MAX_PIPELINE_LOG_BYTES) {
    pipelineLogBytes += text.length + 1
    try { pipelineLogStream.write(`${text}\n`) } catch { /* 日志失败不影响流水线 */ }
    if (pipelineLogBytes > MAX_PIPELINE_LOG_BYTES) {
      try { pipelineLogStream.write('…（日志超过 8MB，后续行不再落盘；界面环形缓冲仍有最近 400 行）\n') } catch { /* 同上 */ }
    }
  }
}

function closePipelineLog() {
  if (pipelineLogStream !== null) {
    try { pipelineLogStream.end() } catch { /* 已关闭 */ }
  }
  pipelineLogStream = null
}
...
/**
 * 从 worktree 的 `.git` 文件回到主仓库根，不依赖 git 命令。
 * worktree 里 `.git` 是文件，内容是 `gitdir: /main/.git/worktrees/<name>`；
 * 三层 dirname 就是 /main。普通仓库 `.git` 是目录，读文件会抛错，交给 git 命令。
 */
function mainWorktreeFromGitFile(dir) {
  try {
    const text = readFileSync(path.join(dir, '.git'), 'utf8').trim()
 ...
 * 优先解析 .git 文件 —— 系统 git 不可用（例如 Xcode 许可未接受）时也能正确回溯。
 */
function resolveMainWorktree(dir) {
  const fromFile = mainWorktreeFromGitFile(dir)
  if (fromFile !== null) return fromFile
  try {
    const common = execFileSync(resolveGitBin(), ['-C', dir, 'rev-parse', '--path-format=absolute', '--git-common-dir'], {
...
/**
 * 从子进程 stderr 里挑一条最有信息量的行带给界面：
 * 优先含 error / fatal / license 的行，否则取最后一行（错误通常压在最后）。
 * 传入的通常是 createLineBuffer().lines —— 有界缓冲区会优先保留这些关键行。
 */
function bestDetail(lines) {
  if (!Array.isArray(lines)) return null
  const cleaned = lines.map((line) => String(line).trim()).filter((line) => line.length > 0)
  if (cleaned.length === 0) return null
  const text = cleaned.find((line) => ERROR_LINE_RE.test(line))
...
/**
 * 工具链预检结果缓存。
 * spawnSync 最多阻塞事件循环 15s，而 /api/update 是同步调用它 —— 每次点更新都
 * 重新探一遍会让整个 webServer 在这段时间里不响应任何请求。
 * 正常结果在进程内永久复用；异常结果只信 TOOLCHAIN_PROBLEM_TTL_MS，
 * 这样用户按提示执行 xcodebuild -license accept 之后不用重启 dsh 也能重试。
 */
let toolchainCache = null
const TOOLCHAIN_PROBLEM_TTL_MS = 60_000

/**
 * 真正跑一次 xcrun 预检。
 * @returns { problem, definitive } —— definitive=false 表示探测本身失败（超时、
 *   spawn 异常），这次结果不可信，调用方不要缓存。
 */
function probeToolchain() {
  try {
    const result = spawnSync('/usr/bin/xcrun', ['-f', 'clang'], { encoding: 'utf8', timeout: 15_000 })
    const output = `${result.stdout ?? ''}${result.stderr ?? ''}`
    if (result.status !== 0 && /Xcode license|not agreed to the Xcode license/i.test(output)) {
      return { problem: 'Xcode 许可尚未接受，原生模块无法编译。请先在终端执行：sudo xcodebuild -license accept', definitive: true }
    }
    return { problem: null, definitive: result.error === undefined }
  } catch {
    // 探测本身失败（例如 xcrun 损坏）：不因此阻断更新，交给后面的构建报真实错误。
    return { problem: null, definitive: false }
  }
}

/**
 * macOS 工具链预检。
 * Xcode 许可没接受时，不只 /usr/bin/git，连 xcrun（clang）都会以 69 退出，
 * 原生模块 build:native-system 必然失败。与其装完依赖、跑两分钟构建再报错，
 * 不如在用户点「立即更新」时就明确拒绝并给出修复命令。
 * @returns 问题描述；工具链正常时返回 null。
 */
function toolchainProblem() {
  if (process.platform !== 'darwin' || !existsSync('/usr/bin/xcrun')) return null
  const now = Date.now()
  if (toolchainCache !== null) {
    const fresh = toolchainCache.problem === null || now - toolchainCache.at < TOOLCHAIN_PROBLEM_TTL_MS
    if (fresh) return toolchainCache.problem
  }
  const probe = probeToolchain()
  if (probe.definitive) toolchainCache = { problem: probe.problem, at: now }
...
function detectRepo() {
  ...
  // runtime.json 记录的主仓库：切换到副本后入口在 slots 里，靠它稳定回到主仓库。
  const recorded = state.runtimeAsset?.mainRepo
  if (typeof recorded === 'string' && recorded.length > 0) candidates.push(recorded)
...
/** 更新通道：默认跟随当前分支的 upstream；DSH_UPDATE_CHANNEL=tag 时跟随最新 dsh-v* 标签。 */
function updateChannel() {
  return process.env.DSH_UPDATE_CHANNEL === 'tag' ? 'tag' : 'master'
}

/**
 * 最新的 dsh-v* 标签（按创建时间倒序）；没有标签时返回 null。
 * readSnapshot 与 resolveTarget 共用它，快照和更新目标才不会各挑一个标签。
 */
...
/**
 * 采集一次快照。`behind` 是「更新目标相对**正在运行的版本**」的提交数：
 * 更新是在副本里做的，主仓库 HEAD 不再代表运行版本。
 *
 * 目标 ref 必须和 resolveTarget() 走同一条通道规则：DSH_UPDATE_CHANNEL=tag 时
 * 比较「最新 dsh-v* 标签」，否则比较分支 upstream。否则 tag 通道下界面会拿
 * 分支的 behind / updateAvailable 去描述一次标签更新，显示的进度是错的。
 */
async function readSnapshot(repo = state.repo) {
  ...
  const base = { repo, upstream: UPSTREAM, channel, originUrl: originUrl.length > 0 ? originUrl : null, originOk }
  ...
  const ref = channel === 'tag' ? await latestReleaseTag(repo) : await upstreamRef(repo)
  // 标签可能是 annotated tag，统一解到 commit，后面的 log / rev-list 才对得上。
  const targetExpr = ref === null ? null : (channel === 'tag' ? `${ref}^{commit}` : ref)
  const [branch, repoHeadSha, remoteSha, repoHeadLine, remoteLine, status, countLine] = await Promise.all([
    git(repo, 'rev-parse --abbrev-ref HEAD'),
    git(repo, 'rev-parse HEAD'),
    targetExpr === null ? Promise.resolve('') : git(repo, `rev-parse ${sh(targetExpr)}`),
    git(repo, "log -1 --format='%h%x1f%s%x1f%cI'"),
    targetExpr === null ? Promise.resolve('') : git(repo, `log -1 --format='%h%x1f%s%x1f%cI' ${sh(targetExpr)}`),
    git(repo, 'status --porcelain'),
    targetExpr === null ? Promise.resolve('') : git(repo, `rev-list --left-right --count HEAD...${sh(targetExpr)}`),
  ])

  if (targetExpr === null || remoteSha.length === 0) {
    return {
      ...base,
      ref: ref ?? null,
      isRepo: true,
      ...
      error: ref === null
        ? channel === 'tag'
          ? '仓库里没有 dsh-v* 标签；可去掉 DSH_UPDATE_CHANNEL=tag 跟随分支'
          : '无法解析上游引用（当前分支没有 upstream，也读不到 origin/HEAD），先执行一次 git fetch origin'
        : channel === 'tag'
          ? `无法解析标签 ${ref} 指向的提交，先执行一次 git fetch origin`
          : `无法解析上游引用（${ref}），先执行一次 git fetch origin`,
    }
  }
  ...
  if (hasActive) {
    const count = await git(repo, `rev-list --count ${sh(active.commit + '..' + targetExpr)}`)
...
async function check() {
  ...
  const fetchErr = createLineBuffer({ maxLines: MAX_STDERR_LINES })
  ...
  if (code !== 0) {
    const detail = bestDetail(fetchErr.lines)
    const reason = detail === null
      ? `git fetch 失败（退出码 ${code}），请检查网络或仓库权限`
      : `git fetch 失败（退出码 ${code}）：${detail}`
    setPhase('error', reason)
    appendLog(`· 检测失败：${reason}`)
    saveCache(snapshot)
 ...
 * @param options.stderr - 传入 createLineBuffer() 的收集器时，把 stderr 行同时收集进去
 *   （用于把真实失败原因写进错误信息）。收集器有上限：pnpm build 能刷几万行。
 */
async function stepBash(script, { cwd, timeoutMs = PIPELINE_TIMEOUT_MS, stderr = null } = {}) {
  ...
  const code = await runBash(script, {
    ...
    onErrLine: (line) => {
      appendLog(line)
      if (stderr !== null && stderr !== undefined) stderr.push(line)
    },
    onSpawn: (child) => {
...
async function resolveTarget(repo) {
  const channel = updateChannel()
  if (channel === 'tag') {
    // 与 readSnapshot 共用 latestReleaseTag：快照比较的目标必须就是这里要构建的目标。
    const tag = await latestReleaseTag(repo)
    if (tag === null) throw new Error('仓库里没有 dsh-v* 标签；可去掉 DSH_UPDATE_CHANNEL=tag 跟随分支')
...
/**
 * 为本次更新创建一个**全新** worktree。
 *
 * 不复用旧目录：旧编译产物会让 bundler 报 MISSING_EXPORT，旧 node_modules 工作区
 * 状态会让 tsdown 去构建不存在的根包入口（Cannot find entry）。全新 worktree +
 * 全新 pnpm install（有 pnpm store，通常几秒）从构造上消除这两类失败。
 */
async function prepareSlot(repo, sha) {
  await git(repo, 'worktree prune')
  const stamp = new Date().toISOString().slice(0, 19).replace('T', '-').replaceAll(':', '')
  const slot = path.resolve(path.join(SLOTS_DIR, `${sha.slice(0, 7)}-${stamp}`))
  if (existsSync(slot)) rmSync(slot, { recursive: true, force: true })
  const code = await stepBash(`git -C ${sh(repo)} worktree add --detach ${sh(slot)} ${sh(sha)}`)
  if (code !== 0) throw new Error(`创建全新试运行副本失败：${slot}`)
  appendLog(`· 全新副本（不复用旧槽位，避免旧产物/旧依赖状态污染）：${slot}`)
  return slot
}

/** 删除不在保留名单里的旧副本，控制磁盘占用；失败不影响更新。 */
async function pruneSlots(repo, keepRoots) {
  const keep = new Set(
    keepRoots
  ...
  for (const candidate of registered) {
    if (!candidate.startsWith(path.resolve(SLOTS_DIR))) continue
    if (keep.has(candidate)) continue
    try {
      const code = await stepBash(`git -C ${sh(repo)} worktree remove --force ${sh(candidate)}`)
      if (code !== 0 || existsSync(candidate)) rmSync(candidate, { recursive: true, force: true })
      appendLog(`· 清理旧副本：${candidate}`)
    } catch {
      // 清理失败不影响更新；下次还会再试。
    }
  }
...
/**
 * 失败分类：把错误 + 日志映射成 kind + 一句可执行建议。
 * 这是「失败后能直接交给模型/agent」的关键——不用让模型重新读几万行日志。
 */
const FAILURE_RULES = [
  {
    kind: 'stale-build-artifacts',
    re: /MISSING_EXPORT|Cannot find entry|Could not resolve|Build failed with \d+ error|TS2307/i,
    hint: '构建踩到旧产物/旧依赖状态（v0.4 起每次更新都用全新 worktree，正常不该出现）；可按报告清理副本后重建',
  },
  {
    kind: 'network',
    re: /SSL_ERROR|Could not resolve host|unable to access|Failed to connect|TLS|Connection (refused|timed out)|Operation timed out|early EOF|RPC failed/i,
    hint: '拉取上游失败（网络/代理/TLS）。确认网络后重试；代理配置见 git config --global --get http.proxy',
  },
  {
    kind: 'toolchain',
    re: /not agreed to the Xcode license|xcrun|Command Line Tools|clang: error/i,
    hint: 'Xcode 许可或命令行工具问题：运行 sudo xcodebuild -license accept（或 brew install git）',
  },
  { kind: 'port', re: /EADDRINUSE|address already in use/i, hint: '端口被占用；确认没有第二个 dsh web 实例' },
  {
    kind: 'plugin-conflict',
    re: /failed to (apply|import) loader entry/i,
    hint: '插件与新版本不兼容；插件会自动写 --patch 隔离，失败报告里列出冲突条目',
  },
  { kind: 'type-error', re: /error TS\d{4}|Type error/i, hint: '上游 TypeScript 类型错误；需要修上游代码或改用 DSH_UPDATE_CHANNEL=tag' },
  { kind: 'disk', re: /ENOSPC|no space left on device/i, hint: '磁盘空间不足；清理 ~/.dsh/dsh-auto-update/slots 与 pnpm store' },
]

function classifyFailure(text) {
  for (const rule of FAILURE_RULES) {
    if (rule.re.test(text)) return { kind: rule.kind, hint: rule.hint }
  }
  return { kind: 'unknown', hint: '未分类失败；按失败报告里的日志与复现命令排查' }
}

/** 组装给模型/agent 的自包含修复提示词。 */
function buildAgentPrompt(bundle) {
  const target = bundle.target ?? {}
  const running = bundle.running ?? {}
  return [
    '你是修复 DeepSeek Harness(dsh) 更新失败的工程代理。',
    '',
    '## 失败',
    `- 步骤：${bundle.step ?? 'unknown'}`,
    `- 错误：${bundle.error ?? 'unknown'}`,
    `- 分类：${bundle.kind}（${bundle.hint}）`,
    `- 目标：${target.ref ?? '?'} @ ${target.sha ?? '?'}（${target.version ?? '?'}，通道 ${target.channel ?? '?'}）`,
    `- 主仓库：${bundle.repo ?? '?'}`,
    `- 失败副本：${bundle.slot ?? '(未创建)'}`,
    `- 正在运行：${running.version ?? '?'} @ ${running.root ?? '?'}`,
    '',
    '## 环境',
    `- node ${bundle.env.node} / ${bundle.env.platform} ${bundle.env.arch} / git ${bundle.env.git}`,
    '',
    '## 复现',
    ...(bundle.reproduce.length > 0 ? bundle.reproduce.map((cmd, index) => `${index + 1}) ${cmd}`) : ['(无)']),
    '',
    '## 要求',
    '1. 先读失败日志（下方 logTail，或完整日志 ' + bundle.logPath + '）确认根因，不要猜；',
    '2. 只在失败副本里工作：可以清理/重建该副本（git worktree remove --force <slot> 后 git worktree add --detach <slot> <sha>），不要改主仓库工作区；',
    '3. 绝对不要重启或杀死正在运行的 dsh web（端口 3080）；',
    '4. 修复后重跑复现命令直到构建成功，并确认 apps/cli/lib/bin.js 与 apps/web/dist/index.html 存在；',
    '5. 结束时输出：根因、所做改动、验证命令与结果、是否建议回到插件点「立即更新」。',
    '',
    '## 失败日志末尾',
    ...(bundle.logTail.length > 0 ? bundle.logTail : ['(无)']),
  ].join('\n')
}

/** 内存日志尾部。 */
function failureLogTail() {
  return state.log.slice(Math.max(0, state.log.length - MAX_FAILURE_TAIL_LINES)).map((entry) => entry.line)
}

/** 写失败报告：failures/latest.json + latest.log，并把摘要挂到 state。 */
function writeFailureBundle({ step, error, target, slot }) {
  const logTail = failureLogTail()
  ...
  const bundle = {
    ...
    slot: slot ?? null,
    target: target ?? null,
    running: summarizeRuntime(state.runtimeAsset?.active),
    env: {
    ...
    reproduce: slot === null || slot === undefined ? [] : [`cd ${slot}`, 'pnpm install', 'pnpm run build'],
    logTail,
    logPath: FAILURE_LOG,
    agentPrompt: '',
  }
  bundle.agentPrompt = buildAgentPrompt(bundle)
  try {
    mkdirSync(FAILURES_DIR, { recursive: true })
    writeFileSync(FAILURE_LOG, logTail.join('\n') + '\n')
    writeJsonAtomic(FAILURE_FILE, bundle)
  } catch (writeError) {
    appendLog(`· 写失败报告出错：${writeError instanceof Error ? writeError.message : String(writeError)}`)
  }
  state.failure = {
    at: bundle.at,
    kind: bundle.kind,
  ...
  saveRuntimeAsset({ lastFailure: { at: bundle.at, kind: bundle.kind, step: bundle.step, slot: bundle.slot } })
  return bundle
}

/** 从磁盘读回失败报告摘要（重启后仍能看到）。 */
function loadFailureBundle() {
  const parsed = readJson(FAILURE_FILE)
  ...
  state.failure = {
    ...
    logPath: typeof parsed.logPath === 'string' ? parsed.logPath : FAILURE_LOG,
  }
  return parsed
}

function clearFailureBundle() {
  state.failure = null
  try { rmSync(FAILURE_FILE, { force: true }) } catch { /* 不存在 */ }
  try { rmSync(FAILURE_LOG, { force: true }) } catch { /* 不存在 */ }
}

/** 解析「让 dsh 帮忙修复」用的入口：受管 active → 当前进程入口。 */
function resolveAssistEntry() {
  const active = state.runtimeAsset?.active?.entry
  if (typeof active === 'string' && existsSync(active)) return active
  const entry = process.argv[1]
  if (typeof entry === 'string' && existsSync(entry)) return entry
  return null
}

/** 起一个后台 dsh 会话（headless profile）尝试修复；不接管正在运行的服务。 */
function startAssist() {
  if (state.assist !== null && state.assist.pid !== null && isProcessAlive(state.assist.pid)) {
    return { ok: false, error: '已经有一个修复会话在运行' }
  }
  const bundle = readJson(FAILURE_FILE)
  if (bundle === null || typeof bundle.agentPrompt !== 'string' || bundle.agentPrompt.length === 0) {
    return { ok: false, error: '没有可用的失败报告；先跑一次更新' }
  }
  const profileDir = path.join(DSH_HOME, 'profiles', ASSIST_PROFILE)
  if (!existsSync(profileDir)) {
    return { ok: false, error: `找不到 profile「${ASSIST_PROFILE}」（${profileDir}）；可用 DSH_UPDATE_ASSIST_PROFILE 指定` }
  }
  const entry = resolveAssistEntry()
  if (entry === null) return { ok: false, error: '无法解析 dsh 入口；可用 scripts/repair.mjs --assist 手动执行' }
  const logPath = path.join(FAILURES_DIR, `assist-${Date.now()}.log`)
  let child
  try {
    mkdirSync(FAILURES_DIR, { recursive: true })
    const fd = openSync(logPath, 'a')
    child = spawn(process.execPath, [entry, '--profile', ASSIST_PROFILE, bundle.agentPrompt], {
      cwd: process.cwd(),
      detached: true,
      stdio: ['ignore', fd, fd],
    })
  ...
  state.assist = { pid: child.pid, logPath, startedAt: nowIso(), error: null }
  try { writeJsonAtomic(ASSIST_FILE, state.assist) } catch { /* 显示用 */ }
  appendLog(`· 已启动 dsh 修复会话（profile ${ASSIST_PROFILE}，pid ${child.pid}）：${logPath}`)
  return { ok: true, pid: child.pid, logPath, profile: ASSIST_PROFILE }
}
...
async function canaryWithQuarantine(slot, target) {
  ...
  for (;;) {
    if (state.cancelRequested) throw new PipelineCancelled()
    // 正常情况下每轮至少隔离一个新条目、由 MAX_QUARANTINE 收敛；这里再兜一层显式上限，
    // 保证将来改动（例如隔离名单不再单调增长）也不会变成无限重试。
    if (attempts >= MAX_CANARY_ATTEMPTS) {
      return { ok: false, entry, error: `试运行重试超过 ${MAX_CANARY_ATTEMPTS} 次上限，已放弃自动隔离`, conflicts: [], attempts }
    }
    attempts += 1
    ...
    // 试运行进程同样登记进子进程集合：/api/cancel 能立刻收掉它，
    // 插件停用/进程退出也不会留下一个攥着空闲端口和真实 profile 的孤儿。
    const bootOnce = () => canaryBoot({
      entry,
    ...
    // 先查预算再写内存：否则内存里的名单会比 runtime.json 多出压线的那一个
    // （界面显示 9 个被禁、实际重启只禁 8 个），状态在下次重启后自相矛盾。
    if (state.quarantine.size + fresh.length > MAX_QUARANTINE) {
...
async function runUpdate(runId) {
  ...
  // 失败报告上下文：出错时把「哪一步、什么目标、哪个副本」一起写进 bundle。
  let failureStep = null
  let failureTarget = null
  let failureSlot = null
  try {
    const mark = (id) => {
    ...
    mark('fetch')
    const fetchErr = createLineBuffer({ maxLines: MAX_STDERR_LINES })
    const fetchCode = await stepBash(`git -C ${sh(repo)} fetch --prune origin`, { timeoutMs: FETCH_TIMEOUT_MS, stderr: fetchErr })
    if (fetchCode !== 0) {
      const detail = bestDetail(fetchErr.lines)
      throw new Error(`git fetch 失败（退出码 ${fetchCode}）${detail === null ? '' : '：' + detail}`)
    }
    ...
    mark('install')
    const installErr = createLineBuffer({ maxLines: MAX_STDERR_LINES })
    const installCode = await stepBash('pnpm install', { cwd: slot, stderr: installErr })
    if (installCode !== 0) {
      const detail = bestDetail(installErr.lines)
      throw new Error(`pnpm install 失败${detail === null ? '' : '：' + detail}`)
    }

    mark('build')
    const buildErr = createLineBuffer({ maxLines: MAX_STDERR_LINES })
    const buildCode = await stepBash('pnpm run build', { cwd: slot, stderr: buildErr })
    if (buildCode !== 0) {
      const detail = bestDetail(buildErr.lines)
      throw new Error(`pnpm run build 失败${detail === null ? '' : '：' + detail}`)
    }
    ...
    // 成功后清掉旧失败报告，并只保留 active + 新副本。
    clearFailureBundle()
    await pruneSlots(repo, [state.runtimeAsset?.active?.root, slot])
    setPhase('done')
  } catch (error) {
    state.finishedAt = nowIso()
    state.step = null
    ...
    const bundle = writeFailureBundle({ step: failureStep, error: message, target: failureTarget, slot: failureSlot })
    appendLog(`· 失败报告：${FAILURE_FILE}（分类 ${bundle.kind}：${bundle.hint}）`)
    appendLog('· 可点设置行里的「复制诊断报告」交给任意 AI 代理，或点「让 dsh 帮忙修复」。')
    try {
      // 保留 active / candidate / 失败副本，方便 agent 直接进副本排查。
      await pruneSlots(repo, [
        state.runtimeAsset?.active?.root,
        state.runtimeAsset?.candidate?.root,
        failureSlot,
      ])
    } catch {
      // 清理失败不影响失败报告。
...
async function startUpdate() {
  ...
  const rejection = (() => {
    ...
    // tag 通道的更新目标是标签、不依赖分支，detached HEAD 也能正常更新。
    if (snapshot.branch === 'HEAD(detached)' && snapshot.channel !== 'tag') {
      return '仓库处于 detached HEAD，无法判断该更新哪个分支（可用 DSH_UPDATE_CHANNEL=tag 跟随标签）'
    }
  ...
  // 工具链预检：Xcode 许可没接受时原生模块必然构建失败，早点说清楚。
  const toolchain = toolchainProblem()
  if (toolchain !== null) {
    setPhase(previousPhase, null)
    appendLog(`· 拒绝更新：${toolchain}`)
    return { ok: false, error: toolchain }
  }
  ...
  appendLog(`仓库 ${state.repo}，通道 ${updateChannel()}，来源 ${snapshot.originUrl ?? '未知'}`)
  appendLog('提示：构建+试运行需要几分钟；期间正在运行的 dsh web 不受影响，可以关闭设置面板。')
  appendLog('每次更新都会新建一个全新副本（不复用旧槽位），避免旧构建产物/旧依赖状态导致构建失败。')
...
function publicState(since = 0) {
  ...
  // 失败报告同理（repair.mjs --clear 或别的进程都可能在宿主之外改它）。
  loadRuntimeAsset()
  loadFailureBundle()
  ...
  return {
    ...
    assist: state.assist === null ? null : {
      running: typeof state.assist.pid === 'number' && isProcessAlive(state.assist.pid),
      pid: typeof state.assist.pid === 'number' ? state.assist.pid : null,
      logPath: typeof state.assist.logPath === 'string' ? state.assist.logPath : null,
      startedAt: typeof state.assist.startedAt === 'string' ? state.assist.startedAt : null,
      error: typeof state.assist.error === 'string' ? state.assist.error : null,
    },
  }
}
 ...
 */
function apply(ctx) {
  // 先读 runtime.json：主仓库路径、被禁插件列表都在里面，detectRepo 也要用。
  loadRuntimeAsset()
  // 失败报告与修复会话状态：重启后在设置行里仍能看到上次为什么失败。
  loadFailureBundle()
  const savedAssist = readJson(ASSIST_FILE)
  if (savedAssist !== null && typeof savedAssist.logPath === 'string') {
    state.assist = {
      pid: typeof savedAssist.pid === 'number' ? savedAssist.pid : null,
      logPath: savedAssist.logPath,
      startedAt: typeof savedAssist.startedAt === 'string' ? savedAssist.startedAt : null,
      error: null,
    }
  }
  ...
  // 缓存必须和当前通道一致：否则切换 DSH_UPDATE_CHANNEL 之后，界面会拿上一次
  // 通道（分支 / 标签）算出来的 behind 去描述另一次更新。
  const cachedChannel = cached?.snapshot?.channel === 'tag' ? 'tag' : 'master'
  if (cached !== null && cached.snapshot !== undefined && cachedChannel === updateChannel()) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (route === '/api/failure' && method === 'GET') {
          // 完整失败报告（含 agentPrompt / logTail），供界面复制或脚本消费。
          const failure = readJson(FAILURE_FILE)
          sendJson(res, 200, { ok: true, failure })
          return
        }
        if (route === '/api/failure/clear' && method === 'POST') {
          if (!isGuardedPost(req)) { sendJson(res, 403, { ok: false, error: '缺少必要的请求标记' }); return }
          clearFailureBundle()
          appendLog('· 已清除失败报告')
          sendJson(res, 200, publicState())
        ...
        if (route === '/api/assist' && method === 'POST') {
          if (!isGuardedPost(req)) { sendJson(res, 403, { ok: false, error: '缺少必要的请求标记' }); return }
          const result = startAssist()
          sendJson(res, result.ok ? 200 : 409, { ...publicState(), ...result })
          return
        }
  ...
  appendLog(`git：${gitBin}`)
  if (/Xcode\.app|CommandLineTools/.test(gitBin)) {
    appendLog('提示：/usr/bin/git 被 Xcode 许可检查拦住，插件已改用 Xcode 自带 git；建议在终端执行 sudo xcodebuild -license accept（或 brew install git）彻底修复。')
  }
