window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /* @dsh-updater:runtime-start */
    /**
     * 运行环境适配层：持久化插件用浏览器原生定时器与 <style> 标签；
     * 动态 Cordis 插件换成 ctx 的 timer 服务与 styles.insert。
     */
    const createRuntime = (ctx) => ({
      schedulePolling: (tick) => {
        const id = setInterval(tick, 1200);
        return () => { clearInterval(id); };
      },
      mountStyles: () => {
        const tag = document.createElement("style");
        tag.dataset.plugin = "@local/dsh-auto-update";
    ...
    /* @dsh-updater:runtime-end */

    /** apply() 时绑定，供组件在渲染期使用。 */
    let runtime = null;

    /* @dsh-updater:transport-start */
    const BASE = "/dsh-updater/api";
    async function post(route, body) {
      ...
      try {
        ...
        return { ok: false, error: text.slice(0, 300) || ("HTTP " + String(res.status)) };
      }
    }
    const transport = {
      state: (since) => fetch(BASE + "/state?since=" + String(since ?? 0)).then((r) => r.json()),
      check: () => post("/check"),
      update: () => post("/update"),
      cancel: () => post("/cancel"),
      restart: () => post("/restart"),
    };
    /* @dsh-updater:transport-end */
    ...
    const zh = {
      ...
      "idle.available": (n) => "发现 " + String(n) + " 个新提交可更新",
      "meta": (version, short, branch) => "当前 " + version + " · " + short + " · " + branch,
      "metaNoVersion": (short, branch) => "当前 " + short + " · " + branch,
      "dirty": (n) => "工作区有 " + String(n) + " 项未提交改动（更新前会自动 stash）",
      "remote": (short, subject) => "上游 " + short + " · " + subject,
      "action.check": "检查更新",
      "action.checking": "检查中…",
      "action.update": "立即更新",
      "action.updating": "更新中…",
    ...
    const en = {
      ...
      "idle.available": (n) => String(n) + " new commit(s) available",
      "meta": (version, short, branch) => version + " · " + short + " · " + branch,
      "metaNoVersion": (short, branch) => short + " · " + branch,
      "dirty": (n) => String(n) + " uncommitted change(s) — stashed automatically before updating",
      "remote": (short, subject) => "upstream " + short + " · " + subject,
      "action.check": "Check",
...
`;
    ...
    function describe(state, t) {
      ...
      if (state.phase === "updating") {
        const step = state.step === null || state.step === undefined ? "" : (t("step." + state.step) || state.step);
        return { text: step.length > 0 ? t("updating") + " · " + step : t("updating"), tone: "brand" };
      }
      if (state.phase === "done") return { text: t("done"), tone: "ok" };
      if (state.phase === "error") return { text: (state.error ?? "").length > 0 ? state.error : t("idle.latest"), tone: "error", error: true };
      ...
      if (snapshot.updateAvailable === true) return { text: t("idle.available")(snapshot.behind), tone: "brand" };
    ...
    function UpdaterRow() {
      ...
      const absorb = useCallback((next) => {
        if (next === null || typeof next !== "object" || next.ok === false) {
          setFailed((next && next.error) || t("error.network"));
          return;
        }
        setFailed(null);
        const incoming = Array.isArray(next.log) ? next.log : [];
        if (incoming.length > 0) {
          sinceRef.current = next.seq;
          setLog((prev) => {
      ...
      const busy = state !== null && (state.phase === "checking" || state.phase === "updating");
      const canUpdate = snapshot !== null && snapshot.isRepo !== false && snapshot.updateAvailable === true && !busy;
      ...
      const detail = (() => {
        ...
        const base = version === null
          ? t("metaNoVersion")(snapshot.head.short, snapshot.branch)
          : t("meta")(version, snapshot.head.short, snapshot.branch);
        return base;
      })();
        ...
        h("div", { className: "dsau-row" },
          h("div", { className: "dsau-text" },
            ...
            snapshot !== null && snapshot.updateAvailable === true && snapshot.remote !== null
              ? h("div", { className: "dsau-meta" }, t("remote")(snapshot.remote.short, snapshot.remote.subject))
              : null,
            snapshot !== null && snapshot.dirty === true
              ? h("div", { className: "dsau-warn" }, t("dirty")(snapshot.dirtyCount))
              : null,
          ...
          h("div", { className: "dsau-actions" },
            ...
            }, state !== null && state.phase === "checking" ? t("action.checking") : t("action.check")),
            updating
              ? h("button", {
                type: "button",
                className: "dsau-btn",
                ...
                onClick: () => {
                  ...
                  void (async () => {
                    try {
                      ...
                      setRestartNote(result !== null && result.ok === true ? t("restart.started") : t("error.network"));
                    } catch {
                      setRestartNote(t("error.network"));
                    }
                  })();
                },
    ...
    /* @dsh-updater:inject-start */
    const inject = ["slots", "locale"];
    /* @dsh-updater:inject-end */
    function apply(ctx) {
