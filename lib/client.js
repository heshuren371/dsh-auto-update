window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 宿主对变更类接口要求这个自定义头；跨站页面加不上它。 */
    const GUARD = { "x-dsh-updater": "1" };
    async function post(route, body) {
      ...
      // 成败以宿主 body 里的 ok 为准，不再看 HTTP 状态码二次推断。
      return parsed;
    }
    const transport = {
      ...
      retryPlugins: () => post("/plugins/retry"),
      // 失败包与 AI 修复会话：GET 拿完整 FailureBundle，POST 请宿主把失败交给代理。
      failure: () => fetch(BASE + "/failure").then((r) => r.json()),
      assist: () => post("/assist"),
    };
